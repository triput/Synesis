...
| [ARCHITECTURE_OVERVIEW.md](docs/ARCHITECTURE_OVERVIEW.md) | How Synesis works under the hood (no code required) |
...
| [**DART_IN_SYNESIS.md**](docs/DART_IN_SYNESIS.md) | Curious engineer tour — Dart/Flutter patterns in *this* repo |
...
**Normal path:** Synesis ships its own Entra **public** client ID ([`lib/auth/oauth_public_clients.dart`](lib/auth/oauth_public_clients.dart)). End users only click **Sign in with Microsoft** — no digging for IDs.

**Overrides (dev / CI):** `--dart-define=SYNESIS_GRAPH_CLIENT_ID=…` → OS env → gitignored `oauth_local.json` (see [`oauth_local.json.example`](oauth_local.json.example)) → shipped defaults.

```bash
flutter run -d windows --dart-define=SYNESIS_GRAPH_CLIENT_ID=YOUR_APP_CLIENT_ID
# optional tenant (defaults to common):
flutter run -d windows --dart-define=SYNESIS_GRAPH_CLIENT_ID=YOUR_APP_CLIENT_ID --dart-define=SYNESIS_GRAPH_TENANT=common
```
...
```bash
flutter run -d windows --dart-define=SYNESIS_GOOGLE_CLIENT_ID=YOUR_GOOGLE_CLIENT_ID
# optional client secret (some Desktop OAuth clients require it):
flutter run -d windows --dart-define=SYNESIS_GOOGLE_CLIENT_ID=YOUR_GOOGLE_CLIENT_ID --dart-define=SYNESIS_GOOGLE_CLIENT_SECRET=YOUR_SECRET
```
...
Register a **public client** (no client secret) in Microsoft Entra ID so Synesis can run authorization code + PKCE in the system browser.
...
2. **Authentication → Add platform**
   - **Mobile and desktop** (or custom):
     ...
     - `synesis://auth` (Android deep link)
...
3. **API permissions → Microsoft Graph → Delegated**
   ...
   - `Calendars.ReadWrite` *(Wave 3 PIM — calendar sync plus RSVP/write; supersedes the prior `Calendars.Read` request)*
   - `offline_access`
   - `openid`
   - `profile`
   - `User.Read`
   ...
   **Re-consent:** Existing Graph accounts signed in before Wave 2 only hold mail scopes; Wave 2 accounts hold the prior read-only calendar scope. After upgrading to a Wave 3+ build, users must **sign in again** (**Edit account → re-auth**) so the consent screen includes `Contacts.Read` and **`Calendars.ReadWrite`** for calendar sync and RSVP/write.
...
```bash
flutter run --dart-define=SYNESIS_GRAPH_CLIENT_ID=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
# optional:
flutter run --dart-define=SYNESIS_GRAPH_CLIENT_ID=... --dart-define=SYNESIS_GRAPH_TENANT=common
```
...
Register OAuth clients in Google Cloud Console so Synesis can run authorization code + PKCE and use Gmail over IMAP/SMTP with XOAUTH2.
...
```bash
flutter run -d windows \
  --dart-define=SYNESIS_GRAPH_CLIENT_ID=... \
  --dart-define=SYNESIS_GOOGLE_CLIENT_ID=...
```

**Notes**

- Redirect ports/schemes are intentionally separate from Microsoft Graph (`8765` / `synesis://auth`).
- **IMAP XOAUTH2** requires the consent-screen scope `https://mail.google.com/` (restricted). If sync fails with `AUTHENTICATIONFAILED` after Sign in with Google, add that scope on the consent screen, enable the Gmail API, turn on IMAP in the Gmail account, then remove the account in Synesis and sign in again so consent is re-granted.
...
| Microsoft Graph OAuth | PKCE browser flow when `SYNESIS_GRAPH_CLIENT_ID` is set |
| Google OAuth | PKCE + XOAUTH2 when `SYNESIS_GOOGLE_CLIENT_ID` is set |
...
- Microsoft Graph: Entra OAuth (PKCE) when `SYNESIS_GRAPH_CLIENT_ID` is set; token paste dev fallback otherwise
- Google: browser OAuth → Gmail IMAP/SMTP XOAUTH2 when `SYNESIS_GOOGLE_CLIENT_ID` is set; app passwords on IMAP tab otherwise
