...
| Pri | ID | Item | Status | V1 wave | Primary files | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| Pri-2 | UI-P21 | **Settings organized by functional area** | **Landed (code)** — 2026-07-23 (V1.5 Wave B) | V1.1+ | `lib/ui/settings/settings_shell.dart`, `settings_catalog.dart`, `settings_sections.dart`, `lib/settings/` | **Post-V1** — not a V1/Final-wave blocker. Replace the growing single long settings scroll with **tabs / section navigation** so functional vs visual settings are not one endless list. Sections: Accounts, Appearance (visual), Reading & message list, Compose, Sync & storage, Notifications, Privacy & security, Shortcuts & accessibility, and Advanced/About. **Navigation pattern:** desktop **tabs or adaptive navigation rail/list** (pick section → show that section’s controls only); mobile **section pages** (list of areas → drill-in). Searchable by setting label/keyword. Retain existing setting values and persistence. **W6 (2026-07-17):** dedicated **Notifications** sheet landed (`notifications_sheet.dart`); full functional-area reorg remains post-V1. **Operator (2026-07-18):** hate scrolling one long settings list — want tabs (or equivalent sectioned nav). Acceptance: a user can identify the owning section from its name, open a section via tab/rail/list (not endless scroll of unrelated controls), search by label/keyword, and reach any setting without scanning the whole surface. **Status (2026-07-23):** old `appearance_sheet.dart` single-scroll sheet removed; new `SettingsShell` opens as a full-screen route (`showSettingsSheet`, `showAppearanceSheet` now a thin alias) with all 9 sections. Desktop/wide (`shortestSide >= 600`) shows a `NavigationRail` beside a single content pane; mobile/narrow shows a section-index `ListTile` list that drills into a dedicated page per section. Top search box filters a static `kSettingsCatalog` of `{label, keywords, sectionId}` entries and jumps to the owning section on tap. Leaf sheets (`manage_accounts_sheet.dart`, `sync_storage_sheet.dart`, `notifications_sheet.dart`) had their bodies extracted to public `*SheetBody` widgets reused by both the old modal entry points and the new sections (no behavior/persistence change); `db_encryption_sheet.dart` and `focus_rules_sheet.dart` stay as separate modals linked from Privacy & security / Reading & message list respectively. Trash retention slider moved from the old Appearance hub into Sync & storage (dedupes the old Appearance-only retention control). Compose section is a short pointer to Manage accounts (no invented compose prefs). Advanced/About uses a soft-coded `kSynesisProductVersion` string (no `package_info_plus` dependency added). Renee QA / full `flutter test` run pending. |
| Pri-2 | UI-P22 | **Search results show message datetime** | **Landed (code)** — 2026-07-23 (V1.5 Wave B) | **W7** / V1.1 | `lib/ui/search/search_sheet.dart`, list projectors | Local and especially **remote/server** search hits must show a clear datetime (`whenLabel` and/or absolute date for older mail) so near-duplicate subjects/senders are distinguishable. Acceptance: every search result row shows a readable sent/received time; older-than-today items use a date (not only “Yesterday”-style relative labels when that would collide). **Status (2026-07-23):** `_SearchSheetBody` made public (`SearchSheetBody`) and each result `ListTile` now shows `msg.whenLabel` as a trailing label; reuses the existing `formatWhen`-derived label (HH:MM today, "Yesterday", else `M/D/Y`) so no new date-formatting logic was needed. Widget test added (`test/search_sheet_test.dart`) covering a same-subject recent-vs-older pair. |
| Pri-2 | UI-P23 | **Folder context menu: mark all read/unread** | **Landed (code)** — 2026-07-23 (V1.5 Wave B) | V1.1+ | `folder_sidebar.dart`, `MailboxCubit.markFolderUnread`, `MessageActionService.markFolderUnread` | Right-click (desktop) / long-press menu on a folder → Mark all as read / Mark all as unread for that folder’s messages (local optimistic + provider push where supported). Acceptance: action completes without opening the folder first; unread badge recounts. **Status (2026-07-23):** `folder_sidebar.dart` `_NavTile` wraps its `InkWell` in a `GestureDetector` handling `onSecondaryTapDown` (desktop right-click) and `onLongPressStart` (mobile long-press) to show a `showMenu` context menu with "Mark all as read"/"Mark all as unread". New `MessageActionService.markFolderUnread({accountId, folderId, unread})` lists folder messages via `MailRepository.listMessages` filtered to the opposite read state (efficiency), bulk-applies via `setUnreadBulk`, enqueues `push_message_action` sync jobs per provider-backed message, then requests a full `MailboxMutationResult(shouldRefresh: true)` so folder badges recount from the DB regardless of which folder/view is currently open. Exposed on `MailboxCubit.markFolderUnread` and wired from `mail_workspace.dart`. Does not touch `setUnreadBulk`'s existing bulk-selection path. Unit tests added in `test/mailbox_cubit_test.dart` (`MailboxCubit markFolderUnread (UI-P23)`). |
...
| Pri-2 | UI-P28 | **Auto-mark settings (delay / off)** | **Landed (code)** — 2026-07-22 (V1.5 Wave A) | V1.1+ | `app_settings_state.dart`, `app_settings_cubit.dart`, `settings_export_service.dart`, `settings_sections.dart`, `reading_pane.dart`, `mail_workspace.dart`, `detached_message_app.dart` | User-configurable auto-mark dwell seconds and ability to disable auto-marking. Default remains 5s / ON when unset. Ties UI-P21 Reading & message list section. Acceptance: setting persists; 0 or Off disables; custom delay used instead of hardcoded 5s. **Status:** `autoMarkAsReadSeconds` (0–60, default 5) added to `AppSettingsState`/cubit/export; "Auto-mark as read" switch + delay slider (originally in the Appearance sheet, moved into the UI-P21 Reading & message list section on 2026-07-23); `ReadingPane` (main + detached windows) resolves dwell/enabled from settings. Renee QA / targeted test run pending. |
| Pri-2 | UI-P29 | **One-click clear active filters** | **Landed (code)** — 2026-07-22 (V1.5 Wave A) | V1.1+ | `message_filter_bar.dart`, `mailbox_cubit.dart` | Dogfood gap after Final-wave Phase B: need a single obvious control (chip × / toolbar Clear) that resets ephemeral `userFilter` in one click without opening the filter sheet. Does **not** delete saved presets. Acceptance: with any active filter, one click restores the unfiltered list; control hidden or disabled when no filter is active. **Status:** plain "Clear" text button replaced with a coral `_ClearFiltersChip` (label + × icon) for discoverability; same one-click `onClearFilters` callback; widget tests updated; unit test added for `MailboxCubit.clearUserFilter`. |
| Pri-1 | UI-P30 | **Hold / pause auto-mark for in-view message (Unread filter)** | **Landed (code)** — 2026-07-22 (V1.5 Wave A) | V1.1+ | `auto_mark_as_read.dart`, `reading_pane.dart`, `mailbox_state.dart` | **Dogfood (2026-07-18):** with **Unread** (or any filter that excludes read) active, the 5s auto-mark (UI-P27) marks the open message read → it drops out of the filtered list and the reading pane closes/advances — jarring while still reading. Need an option to **disable / pause auto-mark for the currently in-view email** (and/or keep selection pinned after auto-mark under restrictive filters). Ties UI-P28 global off/delay. Acceptance: user can keep reading an auto-marked message without the pane vanishing mid-read when Unread filter is on; default V1 behavior may remain until this ships. **Status:** `AutoMarkAsReadController` gained `holdCurrent()`/`releaseHold()`/`isHeld`/`heldMessageId`; reading pane More-menu "Keep unread while reading" toggle + `_AutoMarkHeldChip`; `MailboxState.stickySelectedMessage` keeps the reading pane pinned on the last-selected message when it drops out of a restrictive filter. Renee QA / targeted test run pending. |
