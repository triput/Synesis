...
- [ ] Copy event → Runbox CalDAV calendar: local immediate + remote appears after sync job
- [ ] Copy contact → Runbox CardDAV list: same
- [ ] Graph/Google → DAV and DAV → Graph/Google paths work (DAV side write)
- [ ] Unpushed `local:*` not wiped by full-pull soft-delete
- [ ] Renee **GO** + Page inventory
- [ ] Trish operator **GO** on Runbox
